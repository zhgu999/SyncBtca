...
std::vector<unsigned char> ParseHexString(const char* psz)
{
...
static __thread char buf[128] = {0};

// g++ -o libbbc.so -shared -fPIC bbc.cpp
char *Addr2Hex(const char *addr)
{
...
char *Hex2Addr(const char *hex)
{
    std::vector<uint8_t> vc = ParseHexString(hex);
    memset(buf,0,sizeof(buf));
    buf[0] = vc[0] + '0';
    std::string str;
    Base32Encode(vc.data() + 1,str);
    strcpy(buf + 1, str.c_str());
    return buf;
}